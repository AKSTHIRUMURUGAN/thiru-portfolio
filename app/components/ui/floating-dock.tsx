...
const FloatingDockMobile = ({
  ...
  return (
    <div className={cn("fixed bottom-4 right-4 z-50 md:hidden flex flex-col items-end", className)}>
      <AnimatePresence>
        {open && (
          <motion.div
            layoutId="nav"
            className="flex flex-col gap-2 bg-neutral-900 p-3 rounded-lg shadow-lg"
      ...
      <button
        onClick={() => {
          setOpen(!open);
          console.log("FloatingDockMobile toggled:", open); // Debugging
        }}
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="px-2 py-0.5 whitespace-pre rounded-md  bg-neutral-800  text-white border-gray-200 absolute left-1/2 -translate-x-1/2 -top-8 w-fit text-xs"
            >
              {title}
            </motion.div>
          )}
